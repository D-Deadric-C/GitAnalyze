...
function getRepoLoaderCacheKey(query: string): string {
    return `gitpulse_repo_loader:${query.toLowerCase()}`;
}
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-[#FDFCFB] text-gray-900 gap-4 p-4">
                ...
                <Link href="/" className="mt-4 px-6 py-3 bg-[#F9C79A] text-gray-900 font-bold border-2 border-black rounded-lg hover:bg-amber-400 transition-colors shadow-[3px_3px_0px_rgba(0,0,0,1)]">
                    Back to Home
                </Link>
            </div>
        );
                ...
                <div className="max-w-md w-full p-8">
                    <div className="mb-8 text-center">
                        <motion.div
                            ...
                            className="mb-4 inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-[#FEF9F2] border-2 border-black"
                        >
                            <GitBranch className="w-10 h-10 text-amber-600" />
                        </motion.div>
                        <h2 className="text-2xl font-bold mb-2 text-gray-900">Loading Repository</h2>
