/**
 * Shared GitPulse system prompt builder.
 *
...
export interface GitPulsePromptParams {
  question: string;
  context: string;
...
/**
 * Builds the full GitPulse prompt for a given request.
 * Pure function: accepts data, returns a string. No IO. Fully testable.
 */
export function buildGitPulsePrompt(params: GitPulsePromptParams): string {
...
export function formatHistoryText(
  ...
  return history
    .map((msg) => `${msg.role === "user" ? "User" : "GitPulse"}: ${msg.content}`)
    .join("\n\n");
}
